    ...
    while True:
        ...
        if results.multi_face_landmarks:
            for face_landmarks in results.multi_face_landmarks:
                ...
                # 코 위치를 모터 위치로 직접 매핑 (절대 위치 제어)
                # nose_x 범위: 0 ~ w (640)
                # 모터 범위: MOTOR_MIN ~ MOTOR_MAX (312 ~ 712)
                # 좌우 (M1) - 대칭이므로 기존 방식 OK
                motor_positions[0] = MOTOR_MIN[0] + nose_x * (MOTOR_MAX[0] - MOTOR_MIN[0]) / w

                # 상하 (M3) - 비대칭이므로 중앙 기준으로 분리
                if nose_y < h/2:  # 위쪽 → 아래로 매핑
                    motor_positions[2] = MOTOR_CENTER[1] - (h/2 - nose_y) * (MOTOR_CENTER[1] - MOTOR_MIN[1]) / (h/2)
                else:  # 아래쪽 → 위로 매핑
                    motor_positions[2] = MOTOR_CENTER[1] + (nose_y - h/2) * (MOTOR_MAX[1] - MOTOR_CENTER[1]) / (h/2)
                ...
                print(f"Base 모터: {int(motor_positions[0])}, nose_x: {nose_x}px")
                print(f"Upper_Arm 모터: {int(motor_positions[2])}, nose_y: {nose_y}px")

                # 코 위치 그리기
                cv2.circle(frame, (nose_x, nose_y), 4, (0, 255, 0), -1)

                # 출력할 좌표값
                text = f"({nose_x}, {nose_y})"
                ...
                # 좌표값이 코 아래에 보이도록 y를 약간 아래로 이동
                offset = 15  # 좌표값을 코 아래로 내리는 픽셀 수

                # 중앙 정렬 + 아래로 offset
                text_x = nose_x - text_w // 2
                text_y = nose_y + text_h + offset
        ...
        cv2.imshow("Nose Landmark + Text Below", frame)
